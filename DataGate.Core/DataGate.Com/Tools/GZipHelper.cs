...
namespace DataGate.Com
{
    ...
    public static class GZipHelper
    {
        ...
        /// <returns></returns>
        public static byte[] Compress(this byte[] bytData)
        {
            using (var stream = new GZipStream(new MemoryStream(bytData), CompressionMode.Compress))
            {
                return IOHelper.StreamToBytes(stream);
            }
        }
        ...
        /// <param name="bytData"></param>
        /// <returns></returns>
        public static byte[] Decompress(this byte[] bytData)
        {
            using (var stream = new GZipStream(new MemoryStream(bytData), CompressionMode.Decompress))
            {
                return IOHelper.StreamToBytes(stream);
            }
        }
        ...
        public static void CompressFile(string sourceFile, string destinationFile)
        {
            ...
            try
            {
                ...
                {
                    throw new ApplicationException($"{nameof(checkCounter)} != {nameof(buffer.Length)}");
                }
